...
// photo...) est inchangée.
//
// MODIF : suppression de la pagination et de la section
// "Missions passées" (interface MissionPassee, query associée,
// imports Pagination/usePagination, icônes Briefcase/Calendar
// devenues inutiles, et le CSS correspondant).
// ================================================================
...
export default function EspaceNounou() {
  ...
  // ===== Menu de compte (topbar style Airbnb, maquette v4) =====
        ...
        <div className="profile-card">
          ...
          <div className="badge-row">
            <span className="badge badge-argile">
              <ShieldCheck size={12} /> Nounou vérifiée
            </span>
          </div>
          <div className="stars">
            {[1, 2, 3, 4, 5].map((n) => (
              <Star
                key={n}
                size={16}
                className={n <= Math.round(note) ? "star-ic filled" : "star-ic"}
                fill={n <= Math.round(note) ? "currentColor" : "none"}
              />
            ))}
          </div>
          ...
          <div className="stat-mini-grid">
            <div className="stat-mini">
              <div className="stat-mini-val">{profil?.experience || "—"}</div>
            ...
            <div className="card">
              ...
              <div className="status-row">
                <div className={`status-icon ${profil?.disponible ? "on" : "off"}`}>
                  {profil?.disponible && <span className="pulse-dot" />}
        ...
        <div className="profile-card">
          ...
          {!isEditingProfil && (
            <button className="btn btn-outline" onClick={startEditingProfil}>
              <Edit2 size={13} /> Modifier mon profil
            </button>
          )}
        </div>
  ...
  return (
    <div className="espace-nounou">
      <div className="app-wrap">
        ...
        {/* ---- Topbar style Airbnb (maquette v4) : logo + retour accueil + menu compte ---- */}
          ...
          <div className="topbar-inner">
            ...
            <div className="topbar-right">
              ...
              <div className={`account-menu ${menuOpen ? "open" : ""}`} ref={menuRef}>
                <button className="account-pill" onClick={() => setMenuOpen((o) => !o)}>
                  <MenuIcon size={17} />
                  <span className={`mini-avatar ${hasAgence ? "" : "neutral"}`}>{initiales}</span>
                </button>
                <div className="account-dropdown">
                  <div className="dropdown-item current" onClick={() => setMenuOpen(false)}>
      ...
      <style>{`
        ...
        /* ---------- Fond vivant : blobs flottants ---------- */
        .app-wrap{ position:relative; min-height:100vh; overflow:hidden; }
        .blob{ position:absolute; border-radius:50%; filter:blur(2px); pointer-events:none; opacity:.4; animation: nc-float 12s ease-in-out infinite; }
        .blob-1{ width:340px; height:340px; top:-140px; right:-60px; background:radial-gradient(circle, var(--argile-tint) 0%, rgba(253,236,200,0) 70%); }
        .blob-2{ width:260px; height:260px; bottom:-80px; left:-80px; background:radial-gradient(circle, var(--feuille-bg) 0%, rgba(231,238,224,0) 72%); animation-delay:-5s; }
        @keyframes nc-float{ 0%,100%{ transform:translate(0,0); } 50%{ transform:translate(-10px, 14px); } }

        /* ---------- Topbar style Airbnb ---------- */
        .topbar-v2{ position:relative; z-index:5; background:var(--white); border-bottom:1px solid var(--line); }
        .topbar-v2::after{ content:""; display:block; height:3px; width:100%; background:linear-gradient(90deg, var(--argile), var(--or), var(--feuille)); opacity:.7; }
        ...
        .ghost-circle{ width:38px; height:38px; border-radius:50%; border:1px solid var(--line); background:var(--white); color:var(--encre-soft); display:flex; align-items:center; justify-content:center; cursor:pointer; transition:.2s; }
        .ghost-circle:hover{ background:var(--sable-deep); color:var(--argile-dark); }

        /* ---------- Menu de compte ---------- */
        .account-menu{ position:relative; }
        .account-pill{ display:flex; align-items:center; gap:10px; background:var(--white); border:1px solid var(--line); border-radius:999px; padding:6px 6px 6px 14px; cursor:pointer; box-shadow:0 1px 3px rgba(61,43,31,.08); transition:.2s; font-family:inherit; }
        .account-pill:hover{ box-shadow:0 5px 14px rgba(61,43,31,.16); }
        .account-pill .lucide{ color:var(--encre-soft); }
        .mini-avatar{ width:30px; height:30px; border-radius:50%; background:linear-gradient(135deg, var(--argile), var(--argile-dark)); color:#fff; display:flex; align-items:center; justify-content:center; font-family:'Fraunces', serif; font-weight:700; font-size:12px; }
        ...
        .account-dropdown{
          ...
          box-shadow:0 14px 34px rgba(61,43,31,.18); padding:8px; z-index:50;
          opacity:0; transform:translateY(-8px); pointer-events:none;
          transition:opacity .18s ease, transform .18s ease;
        }
        .account-menu.open .account-dropdown{ opacity:1; transform:translateY(0); pointer-events:auto; }
        .dropdown-item{ display:flex; align-items:center; gap:11px; padding:11px 12px; border-radius:10px; font-size:13.5px; font-weight:700; cursor:pointer; transition:.15s; }
        ...
        .profile-card{ background:var(--white); border-radius:var(--radius-lg); padding:26px 24px; box-shadow:0 2px 10px rgba(61,43,31,.05); transition:transform .25s ease, box-shadow .25s ease; position:relative; overflow:hidden; }
        ...
        .avatar-halo{
          ...
          position:relative; animation: nc-breathe 4.5s ease-in-out infinite;
        }
        @keyframes nc-breathe{ 0%,100%{ box-shadow:0 0 0 0 rgba(247,168,42,.18); } 50%{ box-shadow:0 0 0 8px rgba(247,168,42,0); } }
        .avatar{ width:82px; height:82px; border-radius:50%; background:var(--white); display:flex; align-items:center; justify-content:center; font-family:'Fraunces', serif; font-weight:700; font-size:24px; color:var(--argile-dark); overflow:hidden; position:relative; }
        .avatar.neutral{ color:var(--sable); background:var(--sidebar); }
        .avatar.editable{ cursor:pointer; }
        .avatar img{ width:100%; height:100%; object-fit:cover; border-radius:50%; }
        .avatar-edit-overlay{ position:absolute; inset:0; border-radius:50%; background:rgba(36,27,21,.55); color:#fff; display:flex; align-items:center; justify-content:center; }
        .verified-badge{ position:absolute; bottom:2px; right:calc(50% - 48px); width:24px; height:24px; border-radius:50%; background:var(--feuille); color:#fff; display:flex; align-items:center; justify-content:center; border:2.5px solid var(--white); }

        .profile-name{ font-family:'Fraunces', serif; font-weight:700; font-size:20px; text-align:center; margin:14px 0 6px; }
        ...
        .stat-mini{ background:linear-gradient(160deg, var(--sable-deep), var(--white)); border-radius:12px; padding:12px 8px; text-align:center; transition:.2s; }
        .stat-mini:hover{ background:linear-gradient(160deg, var(--argile-tint), var(--white)); transform:scale(1.03); }
        .stat-mini-val{ font-family:'Fraunces', serif; font-weight:700; font-size:18px; }
        .stat-mini-lbl{ font-size:10.5px; color:var(--muted); margin-top:2px; }

        .btn{ border:none; border-radius:999px; font-weight:700; font-size:13.5px; cursor:pointer; padding:11px 18px; display:inline-flex; align-items:center; gap:7px; justify-content:center; transition:transform .15s ease, box-shadow .15s ease; font-family:'Public Sans', sans-serif; }
        .btn:hover{ transform:translateY(-2px); }
        .btn:disabled{ opacity:.6; cursor:not-allowed; transform:none; }
        .btn-outline{ background:var(--white); color:var(--argile-dark); border:1.5px solid var(--argile); width:100%; }
        ...
        /* ---------- Cartes secondaires ---------- */
        .card{ background:var(--white); border-radius:var(--radius-lg); padding:20px 22px; box-shadow:0 2px 10px rgba(61,43,31,.04); transition:transform .22s ease, box-shadow .22s ease; }
        .card:hover{ transform:translateY(-2px); box-shadow:0 10px 24px rgba(61,43,31,.09); }
        .card-title{ display:flex; align-items:center; gap:9px; font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:.5px; color:var(--muted); margin:0 0 14px; }
        .card-title svg{ color:var(--argile-dark); flex:none; }

        .two-col{ display:grid; grid-template-columns:1fr 1fr; gap:16px; }
        @media (max-width:600px){ .two-col{ grid-template-columns:1fr; } }

        .status-row{ display:flex; align-items:center; gap:13px; }
        .status-icon{ width:42px; height:42px; border-radius:11px; display:flex; align-items:center; justify-content:center; position:relative; flex:none; }
        .status-icon.on{ background:var(--feuille-bg); color:var(--feuille); }
        .status-icon.off{ background:#F6E1DC; color:var(--rouge); }
        .pulse-dot{ position:absolute; top:-3px; right:-3px; width:11px; height:11px; border-radius:50%; background:var(--feuille); animation:nc-pulse 1.8s infinite; }
        @keyframes nc-pulse{ 0%{ box-shadow:0 0 0 0 rgba(94,124,76,.5); } 70%{ box-shadow:0 0 0 8px rgba(94,124,76,0); } 100%{ box-shadow:0 0 0 0 rgba(94,124,76,0); } }
        ...
        .wa-btn{ display:inline-flex; align-items:center; gap:8px; text-decoration:none; background:var(--whatsapp); color:#fff; font-weight:700; font-size:13.5px; padding:12px 20px; border-radius:999px; transition:transform .15s ease, box-shadow .15s ease; box-shadow:0 6px 16px rgba(37,211,102,.3); flex:none; }
        .wa-btn:hover{ background:var(--whatsapp-dark); transform:translateY(-2px); box-shadow:0 10px 22px rgba(37,211,102,.4); }

        /* ---------- Informations (sans agence) ---------- */
        ...
        .info-tile-label{ font-size:10.5px; font-weight:700; text-transform:uppercase; color:var(--muted); letter-spacing:.4px; }
        .info-tile-value{ font-size:14px; font-weight:700; margin-top:5px; word-break:break-word; }
        ...
        .field input, .field select{ font-family:'Public Sans', sans-serif; font-size:14px; padding:10px 12px; border-radius:10px; border:1px solid var(--line); background:var(--sable-deep); color:var(--encre); outline:none; }
        ...
        /* ---------- Agences disponibles (sans agence) ---------- */
        .commune-filter{ display:flex; align-items:center; justify-content:space-between; margin-bottom:14px; flex-wrap:wrap; gap:8px; }
        select{ font-family:'Public Sans', sans-serif; font-size:13px; font-weight:600; color:var(--encre); border:1px solid var(--line); background:var(--sable-deep); border-radius:999px; padding:8px 14px; }
        ...
        .agency-card{ background:var(--sable-deep); border-radius:var(--radius-md); padding:16px; text-align:center; transition:transform .2s ease, box-shadow .2s ease; }
        .agency-card:hover{ transform:translateY(-3px); box-shadow:0 10px 20px rgba(61,43,31,.1); background:var(--white); }
        .agency-icon{ width:44px; height:44px; border-radius:12px; background:var(--white); color:var(--argile); display:flex; align-items:center; justify-content:center; margin:0 auto 10px; transition:.2s; }
        .agency-card:hover .agency-icon{ background:var(--argile-tint); transform:scale(1.08); }
        .agency-name{ font-size:14px; font-weight:700; line-height:1.25; }
        .agency-meta{ font-size:11.5px; color:var(--encre-soft); margin-top:6px; display:flex; justify-content:center; gap:12px; }
        .agency-meta span{ display:inline-flex; align-items:center; gap:4px; }
        .agency-cta{ margin-top:12px; width:100%; border:none; border-radius:999px; padding:9px 10px; font-size:12px; font-weight:700; cursor:pointer; font-family:inherit; transition:.2s; display:flex; align-items:center; justify-content:center; gap:6px; }
        ...
        .agency-cta-cancel{ margin-top:6px; background:transparent; border:none; padding:0; font-family:inherit; font-size:11px; font-weight:700; color:var(--rouge); text-decoration:underline; cursor:pointer; }
        ...
        .tip{ display:flex; gap:11px; background:var(--feuille-bg); border-radius:var(--radius-md); padding:15px 18px; font-size:13px; color:#3F5533; line-height:1.5; align-items:flex-start; }
        .tip svg{ color:var(--feuille); margin-top:1px; flex:none; }
