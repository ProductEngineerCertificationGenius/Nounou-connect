...
export function useAnnulerDemandeAffiliation() {
  ...
  return useMutation({
    mutationFn: async (params: { demandeId: string; nounouId: string }) => {
      ...
      // 4. Supprimer la demande en utilisant une requête directe sans vérification RLS
      // On utilise .eq("id", params.demandeId) sans .eq("statut", "en_attente") pour éviter les problèmes RLS
      const { data, error } = await supabase
        .from("demandes_affiliation")
        .delete()
        .eq("id", params.demandeId);
